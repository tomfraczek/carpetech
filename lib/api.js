import { TOPBAR, SERVICES, HOMEPAGE, SERVICE, NAVIGATION } from '@/lib/queries';
...
function extractNavigation(fetchResponse) {
  return fetchResponse?.data?.navigation.menuItemCollection.items;
}
...
export async function getNavigation(isDraftMode) {
  const entries = await fetchGraphQL(`query ${NAVIGATION}`, isDraftMode);
